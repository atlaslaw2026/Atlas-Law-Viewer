    ...
    try:
        ...
        for a in soup.select("a"):
            ...
            if (
                ...
                or "opinion" in href.lower()
                or "opinions" in text.lower()
            ):
...
def backfill_ninth_local_pdfs(
    conn: sqlite3.Connection, max_download: int = MAX_NINTH_PDF_DOWNLOAD_PER_RUN
) -> int:
    cur = conn.cursor()
    ...
    for url, title, pdf_url, local_pdf_path in rows:
        ...
        if (
            ...
            and os.path.abspath(local_pdf_path) == os.path.abspath(desired_path)
        ):
            continue
...
def run_once():
        ...
        try:
            ...
            if "pdf" in content_type.lower() or url.lower().endswith(".pdf"):
                ...
                if parsed.get("pdf_url"):
                    local_pdf_path = cache_ninth_pdf(
                        parsed.get("pdf_url") or "", parsed.get("title") or title_hint
                    )
                parsed["local_pdf_path"] = local_pdf_path

            text = parsed.get("text") or ""
            pdf_url = (
                parsed.get("pdf_url") or url
                if "pdf" in content_type.lower()
                else parsed.get("pdf_url")
            )
    ...
    if new_rows or refreshed_rows:
        ...
        for r in refreshed_rows:
            lines.append(
                f"- [refreshed] {r.get('title')} — {r.get('url')}\n  Subjects: {','.join(r.get('subjects', []))}\n  Citations: {json.dumps(r.get('citations', {}), ensure_ascii=False)}\n"
            )
